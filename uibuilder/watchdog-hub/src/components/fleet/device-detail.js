            ...
            '<ui-modal :opener="opener" title-id="device-detail-title" description-id="device-detail-description" @close="close">' +
                    ...
                    '<dl class="detail-grid">' +
                        ...
                        '<template v-if="device.protocol === \'modbus\' && device.health && device.health.modbus">' +
                            ...
                            '<dt>File Modbus</dt><dd v-if="device.health.modbus.queue">{{ indicatorLabel(device.health.modbus.queue.state) }}<span v-if="device.health.modbus.queue.pending !== null && device.health.modbus.queue.pending !== undefined"> : {{ device.health.modbus.queue.pending }} en attente</span><span v-if="device.health.modbus.queue.detail">, {{ device.health.modbus.queue.detail }}</span></dd><dd v-else>Non disponible</dd>' +
                        '</template>' +
                        ...
                        '<dt>Store and Forward</dt><dd>{{ indicatorLabel(device.indicators.buffer) }}<span v-if="device.buffer"> : {{ valueOrUnknown(device.buffer.pending) }} en attente<span v-if="device.buffer.detail">, {{ device.buffer.detail }}</span><span v-if="device.buffer.lastError"> : {{ errorMessage(device.buffer.lastError) }}</span></span></dd>' +
