            ...
            '<section class="flows-section" aria-labelledby="flows-list-title" :aria-busy="refreshing ? \'true\' : \'false\'">' +
                ...
                '<div v-if="flows.length" class="table-wrap">' +
                    '<table class="flows-table"><caption class="sr-only">État des flux supervisés</caption>' +
                        '<colgroup><col class="flows-table__col--flow"><col class="flows-table__col--route"><col class="flows-table__col--status"><col class="flows-table__col--diagnostic"><col class="flows-table__col--action"></colgroup>' +
                        '<thead><tr><th scope="col">Flux</th><th scope="col">Source → Destination</th><th scope="col">État</th><th scope="col">Diagnostic</th><th scope="col">Action</th></tr></thead>' +
                        '<tbody><tr v-for="flow in flows" :key="flow.flow_id">' +
                            ...
                            '<td class="technical-cell flow-route" :title="(flow.source_id || \'Non disponible\') + \' → \' + (flow.destination_id || \'Non disponible\')" :aria-label="\'Source \' + (flow.source_id || \'Non disponible\') + \', destination \' + (flow.destination_id || \'Non disponible\')"><code>{{ flow.source_id || \'Non disponible\' }}</code><span aria-hidden="true">→</span><code>{{ flow.destination_id || \'Non disponible\' }}</code></td>' +
